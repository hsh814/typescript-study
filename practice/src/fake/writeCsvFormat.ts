...
import {mkdir, writeFile, appendFile} from '../fileApi/index';
import {range} from '../util/range';

export const writeCsvFormat = async (filename: string, numberOfItems: number): Promise<string> => {
  ...
  return `write ${numberOfItems} items to ${filename}`;
}
